...
import { FeatureBlock } from '../spec/FeatureBlock';
...
const FEATURES: { group: string; blocks: { title: string; desc: string; badge?: string }[] }[] = [
  {
    group: 'Question creation',
    blocks: [
      { title: 'Rich text editor', desc: 'Stem, options, image upload (alt text required), media embed, table. Rationale shown post-submit on low-stakes exams.' },
      { title: '9 question types + OSCE', desc: 'MCQ, multi-select, fill-in-blank, match, hotspot, audio, video, case study, passage. 32 layout variants built.' },
      { title: 'Version control', desc: 'Every edit creates a new immutable version. Exams pin to version at creation; Dept Head endorses per-department version.' },
      { title: 'Accessibility panel', desc: 'Alt text (required, blocks save), TTS hint text, per-question calculator toggle, 4-circle accessibility score.', badge: 'NEW' },
    ],
  },
  {
    group: 'Tagging system',
    blocks: [
      { title: '5 predefined categories', desc: "Topic (3-4 level tree), Bloom's (6 levels), Difficulty (compound with Year/Level), USMLE/Competency, custom tags." },
      { title: 'AI tag assist', desc: "Bloom's auto-suggest on save; tag consistency checker; ExamSoft import auto-tags 10,000+ questions; AI shadow tags." },
      { title: 'Scoped Views on a flat pool', desc: 'Not siloed by course. Tier 1: pinned/recent/courses sidebar. Tier 2: Browse-All slide-out. Auto-created per course.' },
    ],
  },
  {
    group: 'Assessment assembly',
    blocks: [
      { title: 'Section-based structure', desc: 'Multi-section exams with section-level marks and distribution (equal/manual/difficulty-based); total auto-computed.' },
      { title: 'AI blueprint assembly', desc: "Phase 2: AI selects questions satisfying all NCCPA blueprint cells. Touro verbatim: 'a lifesaver'. Target: Cohere Aug." },
      { title: 'Lifecycle stepper', desc: 'Draft → Building → Review → Published → Scheduled → Live; all nodes clickable; Submit-for-Review gates the transition.' },
    ],
  },
  {
    group: 'Accessibility settings',
    blocks: [
      { title: 'Global exam toggles', desc: 'TTS, STT, OSK, focus mode, calculator type, zoom default, display mode — all configurable per exam.', badge: 'NEW' },
      { title: 'Accommodation profiles', desc: "Named program-level profiles, preset + custom, bulk apply to N students. D2L's 70-operation workflow reduced to 1. First-to-market.", badge: 'NEW' },
      { title: 'Publish gate checklist', desc: 'Blocks publish until: all images have alt text, all media has captions, accommodation-flagged students have profiles.', badge: 'NEW' },
    ],
  },
  {
    group: 'Analytics layer',
    blocks: [
      { title: 'Item performance', desc: 'p-value per question per course offering, point biserial, discrimination index; negative PBis auto-flags.' },
      { title: 'PANCE predictor', desc: "Ed Razenbach's Excel model automated: 8 variables, R² 0.66–0.84. PACKRAT + EOR z-scores + GPA + remediations flag at-risk students months early." },
      { title: 'Accommodation analytics', desc: 'Standard vs extended-time cohort comparison, per-accommodation breakdown. Privacy: N ≥ 5 students per group.' },
    ],
  },
];
...
const ANALOGIES = [
  ...
  { name: 'Curriculum mapping', refs: 'Jira epic→story→subtask hierarchy · OKR alignment tools · dependency graphs · Excel pivot tables (the thing to replace)', takeaway: 'A graph/relationship problem, not a table problem: standards are nodes, assessments are edges, missing edges are coverage gaps. Faculty need to see what is unmapped, not just what is mapped.' },
];
...
export function Builder() {
  return (
    <VStack gap={6}>
      ...
      <SpecSection title="Feature map" sub="Six capability groups; NEW marks first-to-market accessibility surfaces.">
        <VStack gap={3}>
          {FEATURES.map((g) => (
            <VStack key={g.group} gap={2}>
              <Text type="label" color="secondary">
                {g.group}
              </Text>
              <Grid columns={{ minWidth: 240, max: 4 }} gap={2}>
                {g.blocks.map((b) => (
                  <FeatureBlock key={b.title} title={b.title} desc={b.desc} badge={b.badge} badgeVariant="success" />
                ))}
              </Grid>
            </VStack>
          ))}
      ...
      <SpecSection title="Feature analogies" sub="Every major feature maps to an enterprise product that already solved the UX at scale — study the pattern, apply the takeaway.">
        <VStack gap={2}>
          {ANALOGIES.map((a) => (
            <Collapsible key={a.name} trigger={a.name} defaultIsOpen={false}>
              <VStack gap={2}>
                <Text type="supporting" as="p" textWrap="pretty">
                  Study: {a.refs}
                </Text>
