  ...
  return (
    <div style={{ padding: '30px 34px 48px', maxWidth: 1080 }}>
      ...
      <nav aria-label="Page acts" className="flex gap-1.5 flex-wrap" style={{ position: 'sticky', top: 0, zIndex: 5, background: 'var(--bg)', padding: '8px 0 10px', marginBottom: 6 }}>
        {ACTS.map(a => (
          <button key={a.id} className="press mono" onClick={() => jump(a.id)} style={{
            fontSize: 10.5, fontWeight: 500, padding: '4px 12px', borderRadius: 12, cursor: 'pointer',
      ...
      <div role="list" aria-label="Key numbers" style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(stats.length, 6)}, 1fr)`, gap: 1, background: 'var(--border)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden', marginBottom: 14 }}>
        {stats.map((s, i) => (
          <div key={i} role="listitem" style={{ background: '#fff', padding: '13px 15px' }}>
            ...
            <span className="mono" style={{ display: 'block', fontSize: 10, color: 'var(--text2)', marginTop: 4 }}>{s.label}</span>
          </div>
        ))}
      </div>
      <blockquote style={{ position: 'relative', margin: '0 0 16px', padding: '2px 0 0 26px', maxWidth: 700 }}>
        ...
        <span className="serif" style={{ fontSize: 15.5, color: 'var(--text)', lineHeight: 1.55, display: 'block' }}>{p.keyQuote}</span>
        <span className="mono" style={{ fontSize: 10, color: 'var(--text2)', marginTop: 5, display: 'block' }}>— {p.keyQuoteSource}</span>
      </blockquote>
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Object.keys(p.dayInLife).length}, 1fr)`, gap: 14, marginBottom: 8 }}>
        {Object.entries(p.dayInLife).map(([pid, day]) => (
          <div key={pid} style={{ borderTop: '2px solid var(--border2)', paddingTop: 9 }}>
            <div className="mono" style={{ fontSize: 10, fontWeight: 600, color: 'var(--text2)', marginBottom: 4 }}>{personaName(pid).toUpperCase()} · TODAY</div>
            <p style={{ fontSize: 12, color: 'var(--text2)', lineHeight: 1.55 }}>{day}</p>
          </div>
        ))}
      </div>
      ...
      <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden', marginBottom: 14 }}>
        <div className="flex items-center justify-between" style={{ padding: '11px 18px', borderBottom: '1px solid var(--border)', background: 'var(--bg)' }}>
          <span style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--text2)' }}>Critical and high findings — open a row for the full artifact</span>
          <button className="press mono" onClick={() => onNav('signals')} style={{ fontSize: 10.5, color: 'var(--accent)', cursor: 'pointer' }}>drill down in Signals →</button>
        </div>
        {shownEvidence.map(i => (
          <div key={i.id} style={{ borderBottom: '1px solid var(--bg3)' }}>
            ...
            {openId === i.id && (
              <div style={{ padding: '0 18px 12px 33px' }}>
                {i.soWhat && <p style={{ fontSize: 12, color: 'var(--text2)', lineHeight: 1.5, marginBottom: 5 }}><span className="mono" style={{ fontWeight: 600, color: 'var(--accent)', fontSize: 9.5 }}>SO WHAT · </span>{i.soWhat}</p>}
                <span className="mono" style={{ fontSize: 9.5, color: 'var(--text3)' }}>{i.source} · {i.createdAt}</span>
              </div>
            )}
          </div>
        ))}
        {topEvidence.length > 6 && (
          <button className="press w-full mono" onClick={() => setShowAll(v => !v)} style={{ padding: '10px 18px', fontSize: 10.5, color: 'var(--text2)', cursor: 'pointer', background: 'var(--bg)', textAlign: 'left' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 8 }}>
        {Object.entries(p.gapsByDiscipline).map(([disc, gaps]) => (
          <div key={disc} style={{ borderTop: '2px solid var(--border2)', paddingTop: 9 }}>
            <div className="mono" style={{ fontSize: 10, fontWeight: 600, color: 'var(--text2)', marginBottom: 5 }}>{disc.toUpperCase()} GAPS · {gaps.length}</div>
            {gaps.map((g, i) => <p key={i} style={{ fontSize: 11.5, color: 'var(--text2)', lineHeight: 1.45, marginBottom: 6 }}>{g}</p>)}
          </div>
        ))}
      </div>
      ...
      <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '15px 18px', marginBottom: 14 }}>
        <div className="mono" style={{ fontSize: 10, fontWeight: 600, color: 'var(--text2)', marginBottom: 5 }}>THE HAPPY PATH THIS PRODUCT IS BUILDING TOWARD</div>
        <div className="flex items-center gap-2 flex-wrap">
          {p.happyPath.split('→').map((step, i, arr) => (
            <span key={i} className="flex items-center gap-2">
              <span style={{ fontSize: 12.5, color: 'var(--text)', lineHeight: 1.4, background: 'var(--bg)', border: '1px solid var(--bg3)', borderRadius: 'var(--radius-sm)', padding: '6px 11px' }}>{step.trim()}</span>
      ...
      <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden', marginBottom: 14 }}>
        ...
        {p.newFeatureFramework.aiOpportunities.map(ai => (
          <div key={ai.feature} className="flex items-start gap-3" style={{ padding: '11px 18px', borderBottom: '1px solid var(--bg3)' }}>
            <span style={{ flex: 1 }}>
              <span style={{ display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>{ai.feature}</span>
              <span style={{ display: 'block', fontSize: 12, color: 'var(--text2)', lineHeight: 1.5, marginTop: 2 }}>{ai.problem}</span>
            </span>
            <span className="mono" style={{ fontSize: 9.5, fontWeight: 600, color: 'var(--accent)', flexShrink: 0, paddingTop: 3 }}>{ai.status.toUpperCase()}</span>
          </div>
        ))}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 8 }}>
        <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
          ...
          {p.amPmPipeline.enhancementRequests.map(r => (
            <div key={r.request} className="flex items-start gap-2.5" style={{ padding: '10px 16px', borderBottom: '1px solid var(--bg3)' }}>
              ...
              <span style={{ flex: 1 }}>
                <span style={{ display: 'block', fontSize: 12.5, color: 'var(--text)', lineHeight: 1.45 }}>{r.request}</span>
                <span className="mono" style={{ fontSize: 9.5, color: 'var(--text2)' }}>{r.requestedBy} · {r.sessions} session{r.sessions > 1 ? 's' : ''}</span>
              </span>
            </div>
          ))}
        </div>
        <div>
          <div style={{ borderTop: '2px solid var(--border2)', paddingTop: 9, marginBottom: 14 }}>
            <div className="mono" style={{ fontSize: 10, fontWeight: 600, color: 'var(--text2)', marginBottom: 5 }}>DESIGN SYSTEM COMPONENTS IN SCOPE</div>
            <div className="flex gap-1.5 flex-wrap">
              {p.newFeatureFramework.designSystemComponents.map(c => (
                <span key={c} className="mono" style={{ fontSize: 10, padding: '2.5px 9px', borderRadius: 3, background: 'var(--bg2)', color: 'var(--text2)', border: '1px solid var(--bg3)' }}>{c}</span>
              ))}
            </div>
          </div>
          <div style={{ borderTop: '2px solid var(--border2)', paddingTop: 9 }}>
            <div className="mono" style={{ fontSize: 10, fontWeight: 600, color: 'var(--text2)', marginBottom: 5 }}>MICRO-INTERACTIONS · CLINICAL-GRADE MOTION</div>
            {p.newFeatureFramework.microInteractions.map((m, i) => (
              <p key={i} style={{ fontSize: 11.5, color: 'var(--text2)', lineHeight: 1.5, marginBottom: 4 }}>{m}</p>
            ))}
          </div>
        </div>
      </div>
          ...
          <div style={{ marginTop: 12 }}>
            {productMilestones.map(m => (
              <div key={m.label} className="flex items-center gap-3" style={{ padding: '6px 0', borderTop: '1px solid var(--bg3)' }}>
                <span className="mono" style={{ fontSize: 10, color: m.d < today ? 'var(--text3)' : 'var(--text2)', width: 88, flexShrink: 0 }}>{m.date}</span>
                <span style={{ fontSize: 12.5, color: m.d < today ? 'var(--text3)' : 'var(--text)' }}>{m.label}</span>
                {m.isHardDeadline && m.d >= today && <span className="mono" style={{ fontSize: 9, fontWeight: 700, color: '#c24d3a' }}>HARD</span>}
                {m.d < today && <span className="mono" style={{ fontSize: 9, color: 'var(--text3)' }}>passed</span>}
              </div>
            ))}
          </div>
        )}
      ...
      <div className="flex items-start gap-14 flex-wrap" style={{ marginBottom: 18 }}>
        <div style={{ borderTop: '2px solid var(--border2)', paddingTop: 9, minWidth: 300, flex: 1 }}>
          <div className="mono" style={{ fontSize: 10, fontWeight: 600, color: 'var(--text2)', marginBottom: 5 }}>DEPENDS ON</div>
          {p.productDependencies.map(d => (
            <button key={d.product} className="press w-full text-left flex items-start gap-2" onClick={() => onNav(d.product)}
              style={{ padding: '5px 0', cursor: 'pointer', fontSize: 11.5, color: 'var(--text2)', lineHeight: 1.5 }}>
              <span className="mono" style={{ color: 'var(--accent)', fontSize: 10.5, flexShrink: 0, paddingTop: 1 }}>{getProduct(d.product)?.shortName}</span>
              <span>{d.dependency}</span>
            </button>
          ))}
        </div>
        <div style={{ borderTop: '2px solid var(--border2)', paddingTop: 9, minWidth: 240 }}>
          <div className="mono" style={{ fontSize: 10, fontWeight: 600, color: 'var(--text2)', marginBottom: 5 }}>COMPETES WITH</div>
          <div className="flex gap-1.5 flex-wrap" style={{ marginBottom: 8 }}>
            {p.competitors.map(c => <span key={c} className="mono" style={{ fontSize: 10, padding: '2.5px 9px', borderRadius: 3, background: 'var(--bg2)', color: 'var(--text2)', border: '1px solid var(--bg3)' }}>{c}</span>)}
          </div>
          <button className="press mono flex items-center gap-1" onClick={() => onNav('competitive')} style={{ fontSize: 10.5, color: 'var(--accent)', cursor: 'pointer' }}>
...
const specBtn = { fontSize: 11, fontWeight: 500, padding: '7px 13px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border2)', background: '#fff', color: 'var(--text2)', cursor: 'pointer' };
