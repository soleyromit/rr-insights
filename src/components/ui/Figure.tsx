...
export function Figure({ title, caption, ariaLabel, children }: {
  ...
  return (
    <figure role="img" aria-label={ariaLabel ?? `${title}. ${caption}`}
      ...
      <div style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--text2)', marginBottom: 10 }}>{title}</div>
      {children}
      <figcaption style={{ fontSize: 13.5, color: 'var(--text2)', lineHeight: 1.45, marginTop: 8, borderTop: '1px solid var(--bg3)', paddingTop: 8 }}>{caption}</figcaption>
    </figure>
  );
}
